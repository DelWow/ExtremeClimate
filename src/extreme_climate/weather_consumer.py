...
class MessageProtocol(Protocol):
    """Kafka message operations used by the consumer boundary."""
...
class ConsumerProtocol(Protocol):
    """Kafka consumer operations required by the processing loop."""

    def subscribe(self, topics: Sequence[str]) -> None: ...

    def poll(self, timeout: float) -> Optional[MessageProtocol]: ...

    def commit(self, *, message: MessageProtocol, asynchronous: bool) -> Any: ...

    def close(self) -> None: ...


class CursorProtocol(Protocol):
    """Database cursor operations used by raw-weather persistence."""

    def fetchone(self) -> Optional[Tuple[Any, ...]]: ...


class ConnectionProtocol(Protocol):
    """Database connection operations required by raw-weather persistence."""
